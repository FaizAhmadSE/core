...
final class ImageThumbnailer
{
    ...
    {
        if (! config('unifilemanager.thumbnails.enabled')
        ...
        if ($dimensions === false || ($dimensions[0] * $dimensions[1]) > (int) config('unifilemanager.thumbnails.max_source_pixels')) {
            return null;
        }
        ...
        $maximum = (int) config('unifilemanager.thumbnails.max_dimension');
    ...
    {
        $root = trim($root, '/');
        $directory = trim((string) config('unifilemanager.thumbnails.directory'), '/');
